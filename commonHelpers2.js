...
</li>`).join("")}const O=document.querySelector(".js-discount-list");O.addEventListener("click",vt);const lt=new g,dt=2;let ut=[],k;async function pt(){k=(await lt.getDiscountedProducts()).slice(0,dt),ft(k,O),f(),ut.forEach(t=>{gt(t._id,u().some(s=>s._id===t._id))})}pt();function gt(e,t){basketButtons.forEach(s=>{s.dataset.itemId===e&&mt(s,t)})}function mt(e,t){const s=document.querySelector(".cart-button");t?(e.innerHTML=`<svg class="ico">
...
<use href="${r}#icon-basket"></use>
      ...
      `).join("")}const _=document.querySelector(".js-popular-list");_.addEventListener("click",Et);const St=new g,$t=5;let bt=[],w;b();async function Pt(){w=(await St.getPopularProducts()).slice(0,$t),Ct(w,_),f(),bt.forEach(t=>{Lt(t._id,u().some(s=>s._id===t._id))})}Pt();function Lt(e,t){basketButtons.forEach(s=>{s.dataset.itemId===e&&It(s,t)})}function It(e,t){const s=document.querySelector(".cart-button");t?(e.innerHTML=`<svg class="cart-icon-pop">
