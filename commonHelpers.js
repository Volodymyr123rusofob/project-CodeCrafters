import{i as h,S as M,u as v,a as g,A}from"./assets/footer-15ddfb3d.js";import"./assets/vendor-6a322d96.js";const D="/project-CodeCrafters/assets/empty-cart-tab-1x-1da79769.png",E="/project-CodeCrafters/assets/empty-cart-desktop-2x-490faf55.png",T="/project-CodeCrafters/assets/empty-cart-tab-1x-1da79769.png",j="/project-CodeCrafters/assets/empty-cart-desktop-2x-490faf55.png",L="/project-CodeCrafters/assets/empty-cart-mob-1x-971d1e28.png",S="/project-CodeCrafters/assets/empty-cart-mob-2x-7db9012b.png",y=()=>`<div class="empty-cart">
...
`,q=e=>`
      ...
      <div class="checkout-input-wrap">
        ...
        `,l=document.querySelector(".container-div-cart-js-one"),m=document.querySelector(".container-div-cart-js-two"),H=document.getElementById("cart-counter-page"),u=new M("productsBasket"),I=new A;async function d(){const e=u.getAllProducts();if(e.length===0)l.innerHTML="",m.innerHTML="",l.insertAdjacentHTML("beforeend",y());else{let $=function(t){t.preventDefault(),w()},w=function(){const t=k.value;if(!t||!P(t)){g("Please enter a valid email address.","error");return}function a(){const r=y();C.innerHTML=r}e.length>0&&a(),g(`Checkout completed! Total Price: $${i.toFixed(2)}.`,"success"),b.textContent="0.00",u.removeAllProducts(),v(),d()},P=function(t){return/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t)};const c=e.map(async({_id:t,quantity:a})=>{try{return t?{...await I.getProductById(t),quantity:a}:(console.error("_id is undefined or empty for a product in the cart"),null)}catch(r){return console.error(`Error fetching product details for _id: ${t}`,r),null}}),o=(await Promise.all(c)).filter(t=>t!==null);l.innerHTML="",l.insertAdjacentHTML("beforeend",_(o));async function p(t){let a=0;return t.forEach(({price:r})=>a+=r),a}const i=await p(o);m.innerHTML="",m.insertAdjacentHTML("beforeend",q(i));async function f(){return{totalPriceElement:document.querySelector(".checkout-total"),emailInput:document.querySelector(".checkout-input-email"),checkoutButton:document.querySelector(".checkout-button"),cartContainer:document.querySelector(".cart-products-list"),price:document.querySelector(".price")}}const{totalPriceElement:b,emailInput:k,checkoutButton:x,cartContainer:C,price:B}=await f();x.addEventListener("click",$)}const s=e.length;H.textContent=`CART (${s})`}d();document.addEventListener("DOMContentLoaded",()=>{document.body.addEventListener("click",async e=>{const s=e.target.closest(".btn-delete-all"),c=e.target.closest(".cart-btn-delete");if(s)u.removeAllProducts(),d();else if(c){const n=c.closest(".cart-product-item").dataset.productId;u.removeProduct(n),d()}v()})});
